...
import { initDb, getAllUsers, upsertUsers, addImage, getImagesByUserId, updateImageRating } from './server/db';
import { initCos, isCosEnabled, uploadImage } from './server/cos';
...
function userDataPlugin(env: EnvVars): Plugin {
  ...
  const dbPath = env.DATABASE_URL
    ...
    : path.resolve(__dirname, 'data', 'verifyvoice.db');
  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      ...
      // --- Learn Q&A API ---
      server.middlewares.use('/api/learn', async (req, res, next) => {
        if (req.url !== '' && req.url !== '/') {
          next();
          return;
        }
      ...
      server.middlewares.use('/api/compare', async (req, res, next) => {
        if (req.url !== '' && req.url !== '/') {
          next();
          return;
        }
